...
class Face_rec:
    def __init__(self,root):
        ...
        f_lbl=Label(self.root,image=self.photoimg_bottom,bg="white")
        f_lbl.place(x=650,y=45,width=950,height=700)


         #----button-----
        b1_1=Button(f_lbl,text="Face Recognition",cursor="hand2",command=self.Face_reco,font=("times new roman",18,"bold"),bg="green",fg="white")
        b1_1.place(x=300,y=500,width=200,height=40)
    ...
    #=====Face recognition=========
    def Face_reco(self):
        def draw_boundray(img,classifier,scaleFactor,miniNeigbhors,color,text,clf):
            gray_image=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
            features=classifier.detectMultiScale(gray_image,scaleFactor,miniNeigbhors)
            ...
            for (x,y,w,h) in features:
                ...
                #separator=''
                #my_n=separator.join(str(x)for x in n)
                n="+".join(n)

                my_cursor.execute("select Roll from student where Student_id="+str(id))
                r=my_cursor.fetchone()
                #separator=''
                #my_r=separator.join(str(x)for x in r)
                r="+".join(r)
                
                my_cursor.execute("select Dep from student where Student_id="+str(id))
                d=my_cursor.fetchone()
                #separator=''
                #my_d=separator.join(str(x)for x in d)
                d="+".join(d)
